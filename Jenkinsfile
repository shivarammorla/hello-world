      ...
      agent any
        ...
        stages{
           ...
           stage('Artifacts to S3'){
			...
			aws elasticbeanstalk create-application-version --application-name "$ApplicationName" --version-label "$BuildName" --description "Build created from JENKINS. Job:$JOB_NAME, BuildId:$BUILD_DISPLAY_NAME, GitCommit:$GIT_COMMIT, GitBranch:$GIT_BRANCH" --source-bundle S3Bucket=$BucketName,S3Key=webapp.war --region ap-south-1
		    }
